...
def parseKml(file):
    ...
    rootName = nameRaw[1:-4]  # "http://earth.google.com/kml/2.2" / "http://www.opengis.net/kml/2.2"
    ...
    for placemark in placemarkList:
        ...
        elif id and id.find('tour') >= 0:
            tracks = placemark.findall(".//{http://www.google.com/kml/ext/2.2}MultiTrack")
            # TODO: handle the files generated by google's MyTrack
...
def filePipeline(filename):             # Process one file
    records = parseKml(filename)

    if np.size(records) > 0:
        title = filename.split('/')

        # TODO: Compute the speed & power measurements
        prettyPlot(records, title[-1])


def folderPipeline(folderName):         # Get all the KML files in this folder and plot

    files = glob.glob(folderName + "/*.kml")
    for file in files:
